...
func SetupScheduleRoutes(api fiber.Router, db *sql.DB, driver string) {
	...
	schedule.Post("/daily-availiblity/fleet", helper.JWTAuthorizationMiddleware(), h.GetDailyAvailabilityFleet)
	schedule.Post("/daily-availiblity/fleet-unit", helper.JWTAuthorizationMiddleware(), h.GetDailyAvailabilityFleetUnit)
