...
func SetupPricingRoutes(app *fiber.App, db *sql.DB, driver string) {
	...
	app.Get("/api/services/packages/pricing", helper.DualAuthMiddleware(orgRepo), h.GetPackages)
	app.Get("/api/services/packages/detail/:package_id", helper.DualAuthMiddleware(orgRepo), h.GetPackageDetail)
